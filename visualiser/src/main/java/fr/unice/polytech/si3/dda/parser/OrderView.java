...
//            count = instruction.execute(tempContext);
            if (counts.containsKey(instruction.getDroneNumber()))
                counts.put(instruction.getDroneNumber(), counts.get(instruction.getDroneNumber()) + instruction.execute(tempContext) - 1);
            else
...
//                PoiStep poiStep = new PoiStep(warehouse.getLast().getInventory(), count);
                ...
                PoiStep poiStep = new PoiStep(deliveryPoint.getLast().getInventory(), counts.get(instruction.getDroneNumber()) - 1);
